...
- React provides 2 options for memoization: `useMemo` (for both variables and
  functions) and`useCallback` (for functions). This helps optimize for repeated
  re-renders due to functions that are having to be initialized within a
  component (which means they are seen as different each time by a `useEffect`
  hook).
